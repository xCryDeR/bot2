...
.setDescription(`<a:hypesquad1:750076071721828452> <@!${message.author.id}> adlı kullanıcı tarafından <#${message.channel.id}> kanalına gönderilen mesaj silindi!\n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Loxo Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Loxo Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
client.on("message", msg => {
  var dm = client.channels.get("723809063149895704"); //KANAL ID GİRİN (Düzenli olması için bu mesajı silebilirsiniz)
              ...
              if (!message.member.hasPermission("ADMINISTRATOR")) {
                  ...
                  if (uyarisayisi === null) {
                      let uyari = new Discord.RichEmbed()
                          .setColor("#ffd100")
                          .setFooter('Loxo BOT', client.user.avatarURL)
                  ...
                  if (uyarisayisi === 3) {
                      message.delete();
                      await kullanici.ban({
                          reason: `Loxo Reklam Kick Sistemi`,
                      })
...
client.on("message", msg => {
  var dm = client.channels.get("769263387392868412")
  if(msg.channel.type === "dm") {
  if(msg.author.id === client.user.id) return;
  const botdm = new Discord.RichEmbed()
  .setTitle(`${client.user.username} Dm`)
